...
Determine the parent commit of the PR, i.e. the SHA in master the PR is based on. The GitHub API reports on the `base` when you `GET` a PR, but that is not the correct SHA, although it seems like it should be do and this baffles me. At this point, I determine the target SHA "manually" by looking at the fork's history in the browser.
