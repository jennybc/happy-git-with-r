...
#### Accessing Git Bash outside of Rstudio (Windows)

Sometimes you will need to run Git Bash outside of RStudio, which can be useful in doing some file operations within a repository. The easiest way to open up a Git Shell outside of RStudio is to click the "Git" menu under the windows menu, and select "Git Bash":

![](img/2018-01_git_bash_windows.png)

Sometimes you will want to run the Git Shell as administrator. You may want to do this because of file permission issues within windows may prevent you from deleting files. The easiest way to do this is to select the "Git" menu item under the startup, and *right-clicking* on Git Bash, which will open up a submenu. Selecting "more" in this submenu will pop up the "Run as Administrator" Option. You'll then get a git bash shell with Administrative privileges. 
...
#### Bash via Windows Services for Linux (For power users only)
...
When in doubt, you probably want to be in a Git Bash shell, either within the terminal tab in or out of RStudio

## Opening the shell outside of Rstudio

Sometimes you might want to open the shell (called Terminal in Mac) outside of Rstudio, or without opening Rstudio.

### MacOS

On a Mac, you can use Spotlight Search (cmd-space) to search "Terminal".

![](img/terminal_mac_search.png)

Opening Terminal brings you to a bash shell opened to your home directory `~/` which is an alias for `/Users/YOURUSERNAME`. You should see a blinking cursor.
...
![](img/2018-01-git-windows-administrator.png)
...
When running commands in the shell you may come across an error telling you that you don't have permission to perform that action. There are ways around this.

#### MacOS

If you have administrative rights on your computer, prefacing any command with `sudo` will ask you force the command to run after asking for your password. If you need to change administrative privileges or your password, [apple support pages](https://support.apple.com/en-us/HT204012) can be helpful.
